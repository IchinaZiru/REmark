...
LEVEL = args.level
assert 0 <= LEVEL <= 10, "--level は 0〜10 を指定してください"
...
# 対象関数だけを抽出するヘルパ
# ------------------------------
import re  # 既にimport済みなら重複OK

def extract_function(code: str, name: str) -> str:
    # 関数シグネチャの先頭を探す
    m = re.search(rf'^[^\S\r\n]*[a-zA-Z_][\w\s\*]*\b{name}\s*\(', code, re.MULTILINE)
    if not m:
        return code  # 見つからなければ全体
    start = m.start()
    # 本体の '{' を探す
...
results = []
quiz_files = sorted(glob.glob("data/mcqa_pools_l10/*.json"))
...
for i, quiz_path in enumerate(quiz_files, 1):
    ...
    func_name = pathlib.Path(quiz_path).stem            
    code_focus = extract_function(code_block, func_name) 
    ...
    if MODEL.startswith("ollama-deepseek"):
            prompt = (
                ...
                "<think>タグも禁止です。\n"
                "出力例: B\n\n"
                f"【対象関数】{func_name} に関する設問です。他の関数は根拠にしないこと。\n"
                f"```c\n{code_focus}\n```\n\n"    # ← code_block → code_focus
                f"問題: {quiz['question']}\n{choice_lines}"
            )
    else:
        prompt = (
            ...
            f"【超重要】この問題は関数 {func_name} の挙動についてのみ判断してください。"
            "他の関数やUI文言は根拠にしないこと。\n\n"
            f"```c\n{code_focus}\n```\n\n"    # ← code_block → code_focus
            f"問題: {quiz['question']}\n{choice_lines}"
        )
    ...
    if MODEL.startswith("claude"):
        ...
        res = client.chat.completions.create(
            ...
            temperature=0,
            top_p=0,      #（決定性を上げる）
            seed=42,      #（再現性補助。対応していない場合は削除）
        )
        answer_raw = res.choices[0].message.content.strip()
        print("fp:", getattr(res, "system_fingerprint", None))
