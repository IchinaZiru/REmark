...
// [Condensing] Enterキーが押されるまでプログラムを一時停止し、メインメニューに戻るよう促す関数です。
void PauseProgram()
{
...
// [Condensing] 画面をクリアし、指定されたタイトルを装飾ラインで囲んで表示します。
...
// [Condensing] ユーザーから時と分を入力させ、stTime構造体として返す関数です。
...
// [Condensing] 2つの日付を比較し、最初の日付が2番目の日付よりも前かを判定します。
...
// [Condensing] 終了画面を表示して、プロジェクトの完了を示します。
void ExitScreen()
{
  system("cls");
  printf("\n\t\t\t\t\t\t*********************************\n");
...
// [Condensing] メインメニューを表示し、ユーザーの選択に基づいて適切な管理メニューを呼び出す関数です。
void MainMenu()
{
...
// [Condensing] ユーザーリストから指定IDのユーザーを検索し、見つかればそのポインタを返す。
...
// [Condensing] 新しいユーザーを既存のユーザーリストの末尾に追加する関数です。
...
// [Condensing] ユーザーIDを入力して該当するユーザーを検索し、結果を表示する関数。
...
// [Condensing] ユーザーリストを表示し、空の場合はその旨を表示する関数です。
...
// [Condensing] ユーザー管理メニューを表示し、選択された操作を実行します。
void UserManagementMenu(UserPtr *UserList)
{
  int Choice = 0;
  do
...
// [Condensing] 部屋の詳細情報をフォーマットしてコンソールに出力する関数です。
...
// [Condensing] ユーザーから部屋のIDと名前を入力し、構造体に格納して初期化する関数です。
...
// [Condensing] 新しい部屋を既存の部屋リストの末尾に追加する関数です。
...
// [Condensing] 指定された名前の部屋をリストから検索し、見つかれば情報を表示して存在フラグを更新します。
...
// [Condensing] 新しい部屋をリストに追加し、重複する場合はエラーメッセージを表示します。
...
// [Condensing] ユーザーが入力した名前で部屋を検索し、見つからない場合はエラーメッセージを表示します。
...
// [Condensing] ユーザーが入力したIDで部屋を検索し、結果を表示する関数です。
...
// [Condensing] 部屋リストを画面に表示し、空の場合はメッセージを表示する関数。
...
// [Condensing] 部屋管理メニューを表示し、ユーザーの選択に応じて部屋の管理操作を実行します。
void RoomManagementMenu(RoomPtr *RoomList)
{
  int Choice = 0;
  do
...
// [Condensing] 予約情報をフォーマットして標準出力に表示する関数です。
...
// [Condensing] 予約情報と部屋情報にユーザー入力から予約期間を設定し、部屋を予約済みにします。
...
// [Condensing] ユーザーから予約情報を取得し、既存のデータと照合して有効な予約を作成します。
...
// [Condensing] 新しい予約を予約リストの末尾に追加する関数です。
...
// [Condensing] 新しい予約を追加し、成功メッセージを表示する関数です。
...
// [Condensing] 指定されたIDの予約を削除し、成功または失敗のメッセージを表示します。
...
// [Condensing] 予約リストを画面に表示し、リストが空の場合はその旨を通知します。
...
// [Condensing] 予約リストを日付順にソートし、結果を表示する関数です。
